...
# 使用MinerU API处理PDF
from slais.pdf_utils_api import convert_pdf_to_markdown, extract_images
...
class PDFParsingAgent:
    ...
    def extract_content(self, pdf_path: str) -> str:
        ...
        expected_md_filepath = output_dir / f"{pdf_name_without_ext}.md"
        ...
        try:
            # 调用转换功能，直接指定最终输出目录
            md_file_path = convert_pdf_to_markdown(pdf_path, output_dir=str(output_dir))
    ...
    def extract_images(self, pdf_path: str, output_dir: str) -> list:
        """
        提取PDF中的图片到指定目录。
        Args:
            pdf_path: PDF文件路径
