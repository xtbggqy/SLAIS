...
请分析以下文献内容，并以 Markdown 格式返回对研究方法的详细评估。
内容应包含以下部分：
...
请仔细阅读以下文献内容，并以 Markdown 格式提取其核心创新点和潜在应用。
内容应包含以下部分：
- **核心创新点**：列出文献的主要创新贡献
- **解决的问题**：描述文献试图解决的核心问题
...
请根据以下文献内容，生成{num_questions}个高质量的问题，涵盖文献的核心概念和关键发现。
...
STORYTELLING_PROMPT = """
请将以下文献内容用讲故事的方式生动地讲述出来。故事应该有趣、简单易懂，同时不失专业性，能够准确传达文献的核心思想、研究过程和主要发现。请避免使用过于技术性的术语，或者在必要时进行解释。
...
MINDMAP_PROMPT = """
请根据以下文献内容，生成一个概括全文逻辑结构的 Mermaid 格式的脑图（思维导图）。
